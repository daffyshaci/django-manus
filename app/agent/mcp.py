...
class MCPAgent(ToolCallAgent):
    ...
    async def initialize(
        ...
        # Add system prompt and available tools information via update_memory
        self.update_memory(
            "system",
    ...
    async def _refresh_tools(self) -> Tuple[List[str], List[str]]:
        ...
        if added_tools:
            ...
            self.update_memory(
                "system", f"New tools available: {', '.join(added_tools)}"
            )
        if removed_tools:
            logger.info(f"Removed MCP tools: {removed_tools}")
            self.update_memory(
                "system", f"Tools no longer available: {', '.join(removed_tools)}"
            )
        if changed_tools:
